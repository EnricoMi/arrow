...
arrow::Result<std::shared_ptr<arrow::Table>> GetTable() {
  std::shared_ptr<arrow::Array> arr_i = ArrayFromJSON(arrow::int32(), "[1, 3, 5, 7, 1]");

  auto struct_type = arrow::struct_({{"a", arrow::int32()}, {"b", arrow::int64()}});
  std::shared_ptr<arrow::Array> arr_struct = ArrayFromJSON(struct_type, "[{[2, 20], [4, 40], [6, 60], [8, 80], [10, 100]}]");

  auto map_type = arrow::map(arrow::int32(), arrow::utf8());
  std::shared_ptr<arrow::Array> arr_map = ArrayFromJSON(map_type, R"([
    [[2, "2"], [4, "4"]],
    [[6, "6"]],
    [],
    [[8, "8"], [10, "10"]],
  ...
  std::shared_ptr<arrow::Array> arr_list = ArrayFromJSON(list_type, R"([
    [1, 2, 3],
    [4, 5, 6],
    [7],
    [8],
